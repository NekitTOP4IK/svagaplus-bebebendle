...
HELP_TEXT = (
    ...
    "• Не больше 6 предложений одновременно на модерации\n\n"
    "СВАГА+ (приоритет в очереди):\n"
    "• Подписка на СУПЕР ПЛАТНЫЙ ТГ-канал (СВАГА+) даёт приоритет "
    "в очереди модерации — предложения разбирают раньше\n"
    "• Если ты уважаемый платный подписчик,"
    " бонус подхватится автоматически "
    "по твоему Telegram — отдельно ничего привязывать в боте не нужно\n"
    "• Проверить статус: /profile "
    "(«Бонус за СВАГУ: активна» = приоритет есть)\n"
    "• На сайте в профиле можно ещё раз сверить подписку СВАГА+ "
    ...
    "8 (800) 333-44-34 (звонок бесплатный, круглосуточно)"
)
