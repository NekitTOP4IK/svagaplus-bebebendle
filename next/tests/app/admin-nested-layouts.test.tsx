...
import AdminAnnouncementsLayout from "@/app/admin/announcements/layout";
import AdminCompetitiveLayout from "@/app/admin/competitive/layout";

describe.each([
  ["announcements", AdminAnnouncementsLayout],
  ["competitive", AdminCompetitiveLayout],
])("Admin %s layout", (_name, Layout) => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("opts into request-time rendering before checking the staff session", async () => {
    const result = await Layout({ children: <div>content</div> });
